...
def main():
    ...
    if uploaded_file is not None:
        ...
        fixed_width, fixed_height = 300, 300  # Fixed size for both images
        ...
        with col2:
            ...
            # Resize saliency map to match the fixed width and height
            saliency_map_resized = cv2.resize(saliency_map, (fixed_width, fixed_height))

            # Plot the resized saliency map
            fig, ax = plt.subplots(figsize=(fixed_width / 100, fixed_height / 100))
            ax.imshow(saliency_map_resized, cmap='jet')
            ax.axis('off')
            st.pyplot(fig)
